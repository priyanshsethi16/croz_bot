...
async function loadExistingUploads() {
  ...
  try {
    ...
    const splitParentStems = new Set();
    pdfs.forEach(name => {
      if (splitRe.test(name)) {
        const root = _rootStem(name);
        (groups[root] = groups[root] || []).push(name);
        splitParentStems.add(root);
        splitParentStems.add(root + '.pdf');
      } else {
        plain.push(name);
      }
    });

    // Render plain PDFs — skip any whose stem already has a split group
    plain.forEach(name => {
      if (!splitParentStems.has(name) && !splitParentStems.has(_rootStem(name))) {
  ...
  list.innerHTML = rows.map((variant, index) => `
    <div class="family-variant-row" data-variant-index="${index}" data-variant-id="${escAttr(variant.variant_id)}">
      <div class="family-variant-grid">
        <input type="text" data-variant-field="name" placeholder="Variant name" value="${escAttr((variant.ordering_data && (variant.ordering_data['Cat_No'] || variant.ordering_data['cat_no'])) || variant.product_code || variant.name)}" />
